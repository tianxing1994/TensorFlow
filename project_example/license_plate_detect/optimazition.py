...
class Optimizer(object):
    def __init__(self, data_loader, model, n_epoch=10, batch_size=32, learning_rate=0.001, show_validation=True):
    ...
    def run(self, ckpt_path=None):
        ...
        for epoch in range(self._n_epoch):
            ...
            for x_train, y_train in self._data_loader(batch_size=self._batch_size, flag="train"):
...
def demo1():
    ...
    model = get_model()
    optimizer = Optimizer(data_loader=batch_load_data, model=model, n_epoch=1000, batch_size=32, learning_rate=0.001)
    optimizer.run()
    return

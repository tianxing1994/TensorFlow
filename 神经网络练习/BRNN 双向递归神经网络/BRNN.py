...
lstm_fw_cell = contrib.rnn.BasicLSTMCell(num_units=128, forget_bias=1.0)
lstm_bw_cell = contrib.rnn.BasicLSTMCell(num_units=128, forget_bias=1.0)

# static_bidirectional_rnn() 的计算是: 将 28 个 (None,28) 输入 lstm_cell, 得出 28 个 (None, 256) 的结果.
# 其中, 每一次的输入每影响下一次输入得出的结果. 最后一次的结果是总结了前面所有输入而得出的结果.
# outputs 为一个长度为 28 的列表, 其形状: [(None,256),(None,256) ... (None,256)]
try:
    outputs, _, _ = contrib.rnn.static_bidirectional_rnn(lstm_fw_cell,lstm_bw_cell, X2, dtype=tf.float32)
except Exception:
    outputs = contrib.rnn.static_bidirectional_rnn(lstm_fw_cell, lstm_bw_cell, X2, dtype=tf.float32)

w1 = tf.Variable(initial_value=tf.random_normal(shape=(256,10)))
b1 = tf.Variable(initial_value=tf.random_normal(shape=(1,10)))

# outputs[-1] 为最后一次输入得出的结果, 形状为 (None, 256).
